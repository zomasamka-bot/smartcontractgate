...
export function PolicyCheck({ request, onComplete, walletConnected = true }: PolicyCheckProps) {
  ...
  const runPolicyChecks = (req: Partial<ContractRequest>): PolicyCheckResult => {
    ...
    // Check 1: Contract address / identifier format
    // Pi Network uses Stellar-style G-addresses (56 chars), UUIDs, or generic identifiers
    const addr = req.contractAddress?.trim() ?? "";
    const isStellar = /^G[A-Z2-7]{55}$/.test(addr);
    const isUUID = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(addr);
    const isEVM = /^0x[0-9a-fA-F]{40}$/.test(addr);
    const isGeneric = addr.length >= 8;
    const addressValid = isStellar || isUUID || isEVM || isGeneric;
    const addressType = isStellar ? "Stellar G-address" : isUUID ? "UUID" : isEVM ? "EVM address" : "Custom identifier";
    checks.push({
      name: "Contract Address / Target Identifier",
      passed: !!addressValid,
      message: addressValid
        ? `Valid ${addressType} accepted`
        : "Address or identifier must be at least 8 characters",
    });
