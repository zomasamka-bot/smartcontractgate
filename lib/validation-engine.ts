...
class ValidationEngine {
  ...
  private validators = {
    ...
    address: (value: string): boolean => {
      ...
      // Pi Network Stellar-style public key: starts with G, 56 base32 characters
      const isStellarAddress = /^G[A-Z2-7]{55}$/.test(trimmed);

      // UUID format (Pi SDK user identifier)
      const isUUID = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(trimmed);

      // EVM-style hex address (for cross-chain or future compatibility)
      const isEVMAddress = /^0x[0-9a-fA-F]{40}$/.test(trimmed);

      // Generic non-empty identifier (minimum 8 characters) as fallback
      const isGenericIdentifier = trimmed.length >= 8;

      return isStellarAddress || isUUID || isEVMAddress || isGenericIdentifier;
    },
